...
const Billings = () => {
  ...
  const [billDate, setBillDate] = useState(new Date().toISOString());
  const [deliveryNote, setDeliveryNote] = useState('');
  const [paymentTerms, setPaymentTerms] = useState('Net 30 days');
  const [buyersOrderNo, setBuyersOrderNo] = useState('');
  const [buyersOrderDate, setBuyersOrderDate] = useState('');
  const [docDispatchNo, setDocDispatchNo] = useState('');
...
CIN : U36996KA2022PTC165134`;
  ...
  const fetchTransportPartners = async () => {
    try {
      ...
      if (Array.isArray(result)) {
        setTransportPartners(result);
        if (result.length > 0) {
          setSelectedTransportPartner(result[0].name); // Select the first partner by default
        }
      } else {
        throw new Error('API response for transport partners was not an array.');
      }
  ...
  const handleSuggestionClick = (order) => {
    ...
    const totalPieces = order.products.reduce((sum, product) => sum + (product.totalPiecesPerBox || 0), 0);
    setTotalPiecesPerBox(totalPieces);
    const deliveryTimestamp = order.statusHistory.find(status => status.status === "Delivered")?.timestamp;
    setDeliveredDate(deliveryTimestamp || '');

    // Set initial values for part 2 fields
    setInvoiceNumber(`INV-${Math.floor(Math.random() * 9000) + 1000}`);
    setBillDate(new Date().toISOString());
    setDeliveryNote(`Total count of: ${totalPieces}`);
    setPaymentTerms('Net 30 days'); // Default
    setBuyersOrderNo(order.orderId);
    setBuyersOrderDate(order.createdAt ? new Date(order.createdAt).toISOString() : '');
    setDocDispatchNo(order.orderId); // Using orderId as doc dispatch no for now
    setDeliveryDate(deliveryTimestamp ? new Date(deliveryTimestamp).toISOString() : '');
    setDestination(order.shippingDetails?.detailedAddress || ''); // Use shipping address as destination
      ...
      setDescriptionItems(
        order.products.map((p, index) => {
          ...
          return {
            id: index + 1,
            purchaseItem: p.productName || `Product ${index + 1}`,
            hsnSac: p.hsnSac || '', // Assuming product might have hsnSac
            qty: qty,
            rate: rate,
            rateType: 'Without GST', // Default to Without GST based on API data structure
            subtotal: subtotal,
  ...
  const handleSaveAndNext = () => {
    ...
    // The final submission will be handled by handleSubmitBill in part 3
  };
  ...
  const handleSubmitBill = async () => {
    if (!selectedOrder) {
      alert("Please select an order before submitting the bill.");
      return;
    }
    if (!isDeclarationChecked) {
      alert("Please agree to the declaration before submitting the bill.");
      return;
    }

   const goodsPayload = descriptionItems.map(item => ({
  purchaseItem: item.purchaseItem,
  hsnSacNo: item.hsnSac,
  qty: item.qty,
  rate: item.rate,
  rateType: item.rateType === 'With GST' ? 'With GST' : 'Without GST', // ✅ ternary fixed
  ...
  totalAmount: parseFloat(item.totalAmount.toFixed(2)),
    ...
    const payload = {
      ...
      sellerDetails: sellerDetails, // Using the hardcoded seller details
      ...
      docDispatchNo: docDispatchNo,
      deliveryDate: deliveryDate,
      dispatchThrough: selectedTransportPartner,
      destination: destination,
      motorVehicleNumber: selectedVehicle,
      billOfLadingNo: billOfLadingNo,
      goods: goodsPayload,
      grandTotal: parseFloat(calculateGrandTotal().toFixed(2)),
      companyBankDetails: companyBankDetails,
      buyerDetails: getBuyerDetailsText(), // Including buyer details for completeness
    };

    console.log("Submitting Bill Payload:", payload); // For debugging

    try {
      const response = await fetch('https://threebapi-1067354145699.asia-south1.run.app/api/billings/add', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.msg || `Failed to add billing. Status: ${response.status}`);
      }

      const result = await response.json();
      alert(`Bill for Order ID ${selectedOrder.orderId} added successfully! Invoice Number: ${result.invoiceNumber}`);
      ...
      <div style={styles.mainContent}>
        ...
        {currentBillingPart === 2 && (
          <div style={styles.nextPartContent}>
            ...
            <div style={styles.inputGroup}>
              <label style={styles.inputGroupLabel}>Invoice Number:</label>
              <input type="text" value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} style={styles.inputField} />
            </div>
            <div style={styles.inputGroup}>
              <label style={styles.inputGroupLabel}>Date and Time of Bill:</label>
              <input type="datetime-local" value={billDate ? new Date(billDate).toISOString().slice(0, 16) : ''} onChange={(e) => setBillDate(new Date(e.target.value).toISOString())} style={styles.inputField} />
            </div>
            <div style={styles.inputGroup}>
              <label style={styles.inputGroupLabel}>Delivery Note:</label>
              <input type="text" value={deliveryNote} onChange={(e) => setDeliveryNote(e.target.value)} style={styles.inputField} />
            </div>
            <div style={styles.inputGroup}>
              <label style={styles.inputGroupLabel}>Mode/Terms of Payment:</label>
              <input type="text" value={paymentTerms} onChange={(e) => setPaymentTerms(e.target.value)} style={styles.inputField} />
            </div>
            <div style={styles.inputGroup}>
              <label style={styles.inputGroupLabel}>Buyer's Order No:</label>
              <input type="text" value={buyersOrderNo} onChange={(e) => setBuyersOrderNo(e.target.value)} style={styles.inputField} />
            </div>
            <div style={styles.inputGroup}>
              <label style={styles.inputGroupLabel}>Dated:</label>
              <input type="date" value={buyersOrderDate ? new Date(buyersOrderDate).toISOString().slice(0, 10) : ''} onChange={(e) => setBuyersOrderDate(new Date(e.target.value).toISOString())} style={styles.inputField} />
            </div>
            <div style={styles.inputGroup}>
              <label style={styles.inputGroupLabel}>Doc Dispatch No:</label>
              <input type="text" value={docDispatchNo} onChange={(e) => setDocDispatchNo(e.target.value)} style={styles.inputField} />
            </div>
            <div style={styles.inputGroup}>
              <label style={styles.inputGroupLabel}>Delivery Date:</label>
              <input type="date" value={deliveryDate ? new Date(deliveryDate).toISOString().slice(0, 10) : ''} onChange={(e) => setDeliveryDate(new Date(e.target.value).toISOString())} style={styles.inputField} />
            </div>
            <div style={styles.inputGroup}>
              <label style={styles.inputGroupLabel}>Dispatch Through:</label>
              <select
            ...
            <div style={styles.inputGroup}>
              <label style={styles.inputGroupLabel}>Destination:</label>
              <input type="text" value={destination} onChange={(e) => setDestination(e.target.value)} style={styles.inputField} />
            </div>
            <div style={styles.inputGroup}>
              <label style={styles.inputGroupLabel}>Motor Vehicle Number:</label>
              <select
            ...
            <div style={styles.inputGroup}>
              <label style={styles.inputGroupLabel}>Bill of Lading/LR-RR NO:</label>
              <input type="text" value={billOfLadingNo} onChange={(e) => setBillOfLadingNo(e.target.value)} placeholder="Optional" style={styles.inputField} />
            </div>
            <div style={styles.buttonContainer}>
              <button onClick={() => setCurrentBillingPart(1)} style={styles.prevButton}>Prev</button>
              <button onClick={handleSaveAndNext} style={styles.saveNextButton}>Save & Next</button>
        ...
        {currentBillingPart === 3 && (
          <div style={styles.nextPartContent}>
            ...
            <div style={styles.buttonContainer}>
              <button onClick={() => setCurrentBillingPart(2)} style={styles.prevButton}>Prev</button>
              <button onClick={handleSubmitBill} style={styles.saveNextButton} disabled={!isDeclarationChecked}>Submit Bill</button>
            </div>
          </div>
        )}
