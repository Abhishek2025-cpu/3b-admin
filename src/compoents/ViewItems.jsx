...
const BoxIcon = () => (
  <svg
    ...
    fill="currentColor"
  >
    <path d="M2 4a2 2 0 012-2h12a2 2 0 012 2v12a2 2 0 01-2 2H4a2 2 0 01-2-2V4zm2-1a1 1 0 00-1 1v2h14V4a1 1 0 00-1-1H4zM3 9v9a1 1 0 001 1h12a1 1 0 001-1V9H3z" />
  </svg>
);
const DeleteIcon = () => (
  <svg
...
const ProgressIcon = () => (
  ...
  >
    <path
      ...
      d="M3 13.125L21 13.125M3 13.125v4.875L21 18V13.125M3 13.125L7.5 4.125M21 13.125L16.5 4.125M7.5 4.125C7.5 3.076 8.336 2.25 9.38 2.25h5.24C15.664 2.25 16.5 3.076 16.5 4.125V13.125L7.5 13.125M7.5 4.125h9V13.125h-9V4.125Z"
    />
  </svg>
);
...
const GenericModal = ({
  ...
  return (
    <div
      className={`fixed inset-0 ${zIndex} flex justify-center items-center p-2 sm:p-4 print:p-0`}
    >
      <div
      ...
      ></div>
      <div
        className={`${maxWidth} bg-white rounded-2xl shadow-2xl w-[95%] sm:w-full max-h-[95vh] flex flex-col relative z-10 overflow-visible print:max-h-none print:shadow-none print:rounded-none`}
      >
        {children}
      </div>
    </div>
...
const UpdateBoxesModal = ({ isOpen, onClose, item, onUpdateSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!boxCount || boxCount <= 0) {
      toast.error("Please enter a valid, positive number of boxes.");
      return;
    }
  ...
  return (
    <GenericModal isOpen={isOpen} onClose={onClose}>
      <div className="p-4 border-b flex justify-between items-center">
        <h2 className="text-xl font-bold text-gray-800">Add More Boxes</h2>
        <button
          onClick={onClose}
      ...
      <form onSubmit={handleSubmit}>
        <div className="p-4 sm:p-6 space-y-4">
          ...
          <div>
            <label
              htmlFor="new-boxes-input"
              className="font-semibold text-gray-700 block mb-1"
            >
              Number of New Boxes to Add
            </label>
            <input
              id="new-boxes-input"
              type="number"
              ...
              onChange={(e) => setNumberOfNewBoxes(e.target.value)}
        ...
        <div className="p-4 border-t flex justify-end gap-3">
          <button
            type="button"
            onClick={onClose}
            className="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-6 rounded-lg w-full sm:w-auto"
          >
            Cancel
          </button>
          <button
            ...
            className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-6 rounded-lg disabled:bg-indigo-300 disabled:cursor-not-allowed w-full sm:w-auto"
          >
            {isSubmitting ? "Adding..." : "Add Boxes"}
          </button>
        </div>
      </form>
    </GenericModal>
...
const PrintablePageLayout = ({ item, box }) => {
  ...
  return (
    <div
      className="border-[4px] border-purple-800 bg-white w-[152.4mm] h-[104mm] relative font-sans text-black mx-auto overflow-visible flex flex-col box-border flex-shrink-0"
      style={{
      ...
      <div className="flex h-full w-full">
        ...
        <div className="w-[62%] flex flex-col justify-between pr-5 h-full pt-1">
          ...
          <div className="flex justify-between items-start w-full">
            {" "}
            {/* items-start rakha hai taaki manually control kar sakein */}
            {/* Logo & Brand Block */}
            ...
            <div className="flex-shrink-0 mr-1 mt-7">
              {" "}
              {/* mt-9 se QR code logo ke barabar niche aa jayega */}
              <img
                src={box.qrCodeUrl}
                alt="Box QR"
          ...
          <div className="flex flex-col space-y-6 mb-12 w-full ml-[2px]">
            <div className="flex items-end w-full">
              <span className="font-extrabold text-[15px] w-[45%] text-left">
                Profile Code
              </span>
            ...
            <div className="flex items-end w-full">
              ...
              <div className="w-[55%] border-b-2 border-[#1349a8] text-center text-[16px] font-extrabold pb-0.5 text-black">
                ...
                {item.length
                  ? (parseFloat(item.length) * 0.3048).toFixed(1)
                  : ""}
              </div>
            </div>
            <div className="flex items-end w-full">
  ...
  justify-content: flex-start !important;
      ...
      >
        <div className="p-4 border-b flex justify-between items-center no-print bg-white">
          ...
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-red-600 text-3xl font-light p-2"
          >
            &times;
          </button>
        </div>
        ...
        <div
          id="printable-area"
          className="bg-gray-50 flex justify-start md:justify-center items-center p-4 print:p-0 overflow-x-auto overflow-y-hidden"
        >
        ...
        <div className="no-print p-4 bg-gray-50 flex flex-col sm:flex-row justify-end gap-3 border-t flex-shrink-0">
          <button
            onClick={handlePrint}
            className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-lg flex justify-center items-center gap-2 shadow-lg w-full sm:w-auto"
          >
            <PrintIcon /> Print / Save PDF
          </button>
          <button
            onClick={onClose}
            className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-6 rounded-lg w-full sm:w-auto"
          >
            Close
          </button>
        </div>
      ...
      >
        <div className="p-4 border-b flex justify-between items-center no-print bg-white">
          <h2 className="text-xl font-bold text-gray-800">
            Print All Boxes ({item.boxes?.length})
          </h2>
          <button
            onClick={onClose}
        ...
        <div
          ...
          className="p-4 overflow-y-auto overflow-x-auto max-h-[70vh] bg-gray-100 print:bg-white print:p-0 print:overflow-visible print:max-h-none print:block"
        >
          ...
          <div className="flex flex-col items-start md:items-center gap-4 min-w-max print:block">
            {item.boxes?.map((box) => (
              <div
                key={box._id}
                className="printable-page border border-gray-300 shadow-sm print:shadow-none print:border-0"
              >
                <PrintablePageLayout item={item} box={box} />
              </div>
            ))}
        ...
        <div className="no-print p-4 bg-gray-50 flex flex-col sm:flex-row justify-end gap-3 border-t">
          <button
            onClick={handlePrint}
            className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-lg flex items-center justify-center gap-2 w-full sm:w-auto"
          >
            <PrintIcon /> Print All
          </button>
          <button
    ...
    >
      ...
      <div className="relative flex-grow flex items-center justify-center p-4">
        ...
        <img
          ...
          className="max-w-full max-h-[calc(70vh-100px)] object-contain rounded-lg shadow-md"
        />
      </div>
      {images.length > 1 && (
    ...
    <GenericModal isOpen={isOpen} onClose={onClose}>
      <div className="p-4 border-b flex justify-between items-center">
        <h2 className="text-lg sm:text-xl font-bold text-gray-800">
          Boxes for Item:{" "}
          <span className="text-indigo-600">{item?.itemNo?.trim()}</span>
        </h2>
        <button
          onClick={onClose}
          className="text-gray-500 hover:text-gray-800 text-3xl"
        ...
        {item?.boxes?.length > 0 ? (
          <ul className="space-y-3">
            {item.boxes.map((box) => (
              <li
                key={box._id}
                className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-gray-50 rounded-lg border gap-3 sm:gap-2"
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm sm:text-lg font-mono text-black truncate">
                      {box.boxSerialNo}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0 self-end sm:self-auto w-full sm:w-auto mt-2 sm:mt-0">
                  <button
                    ...
                    className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-3 rounded-lg transition-colors"
                    title="Track"
                  >
                    <TrackIcon />
                    <span className="sm:hidden md:inline text-sm">Track</span>
                  </button>
                  <button
                    onClick={() => onOpenPrintModal(box)}
                    className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gray-600 hover:bg-gray-700 text-white font-semibold py-2 px-3 rounded-lg transition-colors"
                    title="View/Print"
                  >
                    <PrintIcon />
                    <span className="sm:hidden md:inline text-sm">Print</span>
                  </button>
                </div>
              </li>
            ))}
      ...
      <div className="p-4 border-t flex flex-col sm:flex-row sm:justify-between items-center gap-3">
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          <button
            ...
            className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-6 rounded-lg w-full sm:w-auto"
          >
            Update
          </button>
          <button
            onClick={onOpenPrintAllModal}
            className="bg-teal-500 hover:bg-teal-600 text-white font-bold py-2 px-6 rounded-lg flex items-center justify-center gap-2 w-full sm:w-auto"
          >
            <PrintIcon /> Print All
          </button>
        </div>
  ...
  return (
    <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 bg-gray-50 text-sm sm:text-base">
      <div>
        ...
        <p className="break-words">{item.length}</p>
      </div>
      <div>
        <p className="font-semibold text-gray-700">Shift:</p>
        <p className="break-words">{item.shift}</p>
      </div>
      <div>
        <p className="font-semibold text-gray-700">Company:</p>
        <p className="break-words">{item.company}</p>
      </div>
      <div>
      ...
      <div>
        ...
        <p className="break-words">
          {item.operators?.length > 0
            ? item.operators.map((op) => op.roleEid).join(", ")
            : "N/A"}
        </p>
      </div>
      ...
      <div>
        ...
        <p className="break-words">
          {item.helpers?.length > 0
            ? item.helpers.map((h) => h.roleEid).join(", ")
            : "N/A"}
        </p>
      </div>
      ...
      <div className="col-span-full mt-2">
        <button
          ...
          className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center w-full sm:w-auto gap-2"
        >
...
const ProgressModal = ({ isOpen, onClose, machineId }) => {
  ...
  const fetchMachineProgress = useCallback(async (id) => {
    ...
    try {
      ...
      const machineAssignment = data.data.find(
        (assignment) => assignment.machine._id === id,
      );
    ...
    >
      ...
      <div className="flex-grow p-4 overflow-y-auto">
        {isLoadingProgress ? (
          ...
          <div className="space-y-6">
            <h3 className="text-lg font-semibold text-gray-800">
              ...
              <span className="text-indigo-600">
                {progressData.machine?.name}
              </span>{" "}
              (Type: {progressData.machine?.type})
            </h3>
            {/* Information blocks... */}
            {progressData.operatorTable &&
            progressData.operatorTable.length > 0 ? (
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {progressData.operatorTable.map((row, index) => (
                      <tr key={index}>
                        ...
                        <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900">
                          {row.frameLength && row.frameLength.length > 0
                            ? row.frameLength.join(", ")
                            : "N/A"}
                        </td>
                        <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900">
                          {row.numberOfBox}
            ...
            {progressData.operatorImages &&
            progressData.operatorImages.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {progressData.operatorImages.map((imgUrl, index) => (
                  <img
                    ...
                    className="w-20 h-20 sm:w-24 sm:h-24 object-cover rounded-lg shadow-sm cursor-pointer hover:shadow-md transition-shadow"
                    onClick={() =>
                      handleOpenImageSlider(progressData.operatorImages)
                    }
                  />
                ))}
              </div>
          ...
          <p className="text-center text-gray-500 py-8">
            Select an item to view its progress.
          </p>
        )}
      </div>
...
function ViewItems() {
  ...
  const [selectedMachineIdForProgress, setSelectedMachineIdForProgress] =
    useState(null);
  ...
  const normalizeItem = (item) => {
    ...
    return {
      ...
      helpers: Array.isArray(base.helpers)
        ? base.helpers
        : Array.isArray(item.helpers)
          ? item.helpers
          : [],
      operators: Array.isArray(base.operators)
        ? base.operators
        : Array.isArray(item.operators)
          ? item.operators
          : [],
      mixtures: Array.isArray(item.mixtures)
        ? item.mixtures
        : Array.isArray(base.mixtures)
          ? base.mixtures
          : [],
      boxes: Array.isArray(item.boxes)
        ? item.boxes
        : Array.isArray(base.boxes)
          ? base.boxes
          : [],
      boxCount:
        base.boxCount ||
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const normalizedDetailData = (
        Array.isArray(detailJson) ? detailJson : detailJson.data || []
      ).map(normalizeItem);
  ...
  const handleUpdateSubmit = async (itemId, numberOfNewBoxes) => {
    setIsLoading(true);
    const promise = fetch(
      `https://threebapi-1067354145699.asia-south1.run.app/api/items/${itemId}/add-boxes`,
      {
        method: "PATCH",
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 flex-1">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  ...
                  <span className="font-semibold text-red-600">{itemNo}</span>?
                </p>
              </div>
            </div>
          </div>
            ...
            <button
              onClick={() => {
                ...
                const deletePromise = fetch(
                  `https://threebtest.onrender.com/api/items/delete-items/${itemId}`,
                  { method: "DELETE" },
                ).then((res) => {
                  if (!res.ok) {
                    return res.json().then((err) => {
  ...
  if (error)
    return (
      <div className="text-center p-8 text-red-500 bg-red-50">
        Error: {error}
      </div>
    );
  ...
  return (
    <>
      ...
      <div className="bg-white shadow-xl rounded-2xl p-4 sm:p-8 w-full mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          ...
          <input
            ...
            className="w-full sm:w-64 p-2 border rounded-xl focus:ring-indigo-500 focus:border-indigo-500"
          />
        </div>
              ...
              {filteredItems.map((item) => (
                <React.Fragment key={item._id}>
                  <tr className="bg-white border-b hover:bg-gray-50">
                      ...
                      item.productImageUrl.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {item.productImageUrl
                            ...
                            .map((imgUrl, index) => (
                              <img
                                key={index}
                                src={imgUrl}
                                alt={`${item.itemNo} product`}
                                className="w-12 h-12 sm:w-16 sm:h-16 object-cover rounded cursor-pointer border border-gray-200 hover:border-indigo-500 transition-all"
                                onClick={() =>
                          ...
                          {item.productImageUrl.length > 3 && (
                            <div
                              className="w-12 h-12 sm:w-16 sm:h-16 flex items-center justify-center bg-gray-100 rounded text-gray-600 text-xs font-semibold cursor-pointer border border-gray-200 hover:border-indigo-500 transition-all"
                              onClick={() =>
                                handleOpenImageSlider(item.productImageUrl, 3)
                              }
                            >
                              +{item.productImageUrl.length - 3}
                            </div>
                    ...
                    <td className="px-6 py-4 text-center font-mono text-lg whitespace-nowrap">
                      {item.boxCount}
                    </td>
                    <td className="px-6 py-4 text-center whitespace-nowrap">
                      <div className="flex items-center justify-center gap-2 sm:gap-4">
                        <button
                          ...
                          title="View Details"
                          className="text-blue-600 hover:text-blue-800 p-1.5 sm:p-1 rounded-full hover:bg-blue-100 transition-colors"
                        >
                          <ViewIcon />
                        </button>
                        <button
                          onClick={() => handleOpenBoxesModal(item)}
                          title="View Boxes"
                          className="text-green-600 hover:text-green-800 p-1.5 sm:p-1 rounded-full hover:bg-green-100 transition-colors"
                        >
                          <BoxIcon />
                        </button>
                        <button
                          onClick={() =>
                            handleDeleteItem(item._id, item.itemNo.trim())
                          }
                          title="Delete Item"
                          className="text-red-600 hover:text-red-800 p-1.5 sm:p-1 rounded-full hover:bg-red-100 transition-colors"
                        >
                          <DeleteIcon />
                        </button>
                      </div>
      ...
      <PrintAllBoxesModal
        ...
        onClose={handleClosePrintAllModal}
        item={selectedItemForBoxes}
      />
      <ImageSliderModal
